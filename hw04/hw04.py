...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.dollar < self.price:
            return f'Please update your balance with ${self.price-self.dollar} more funds.'

        self.quantity -= 1
        if self.dollar > self.price:
            change = self.dollar-self.price
            self.dollar = 0
    ...
    def add_funds(self, dollar):
        if self.quantity == 0:
            return f'Nothing left to vend. Please restock. Here is your ${dollar}.'
        self.dollar += dollar
