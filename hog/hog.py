...
def oink_points(player_score, opponent_score):
    ...
    opponent_score_af=opponent_score//10
    d2=opponent_score_af%10
    return max(d2*2-d1, 1)
    # END PROBLEM 2
...
def pigs_on_prime(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    if player_score==1 or player_score==2:
        return 1
...
def announce(score0, score1, last_leader=None):
    ...
    if leader == last_leader:
        ...
        message = f"Player {leader} takes the lead by {diff}"
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0,score1) < goal:
        if who==0:
            player_strategy = strategy0(score0,score1)
            score0   += take_turn(player_strategy, score0, score1, dice, goal)
            score0   += pigs_on_prime(score0, score1)
       
            
        else:
            player_strategy = strategy1(score1,score0)
            score1   += take_turn(player_strategy, score1, score0, dice, goal)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for num_rolls in range(1, 11):
        ...
        avrg_point = averaged_dice(num_rolls, dice)
        if avrg_point > std:
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_after = score + oink_points(score, opponent_score)
    if pigs_on_prime(score_after, opponent_score):
        return 0
    
    score_after += pigs_on_prime(score_after, opponent_score)
    if (score_after - score) >= threshold:
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    th_oink = 10 #相手の阻む
    threshold = 8
    if (score - opponent_score) > th_diff:
        return 1
