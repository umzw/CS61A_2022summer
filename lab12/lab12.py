...
def add_trees(t1, t2):
    ...
    if length_t1 < length_t2:
        t1_branches += [0 for _ in range(length_t1, length_t2)]
    elif length_t1 > length_t2:
        t2_branches += [0 for _ in range(length_t2, length_t1)]
    return Tree(new_label, [add_trees(l1, l2) for l1, l2 in zip(t1_branches, t2_branches)])
...
class AggressivePlayer(Player):
    ...
    def choose(self, other):
        "*** YOUR CODE HERE ***"
        if other.popularity >= self.popularity:
            return self.debate
        return self.speech
